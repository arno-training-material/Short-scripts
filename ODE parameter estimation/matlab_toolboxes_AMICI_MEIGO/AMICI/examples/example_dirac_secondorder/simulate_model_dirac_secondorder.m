% simulate_model_dirac_secondorder.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_model_dirac_secondorder(tout,theta)
% [...] = simulate_model_dirac_secondorder(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_model_dirac_secondorder(...)
%
% INPUTS:
% =======
...
function varargout = simulate_model_dirac_secondorder(varargin)
...
sol = ami_model_dirac_secondorder(tout,theta(1:4),kappa(1:0),options_ami,plist,pbar,xscale,data);
else
...
if(options_ami.sensi == 2)
    ...
    s2z = zeros(size(sol.z,1),0,length(theta(options_ami.sens_ind)),length(theta(options_ami.sens_ind)));
    for iz = 1:0
    ...
    sol.s2z = bsxfun(@times,s2z,permute(theta(options_ami.sens_ind)*transpose(theta(options_ami.sens_ind))*(log(10)^2),[4,3,2,1])) + bsxfun(@times,sz,permute(diag(log(10)^2*theta(options_ami.sens_ind).*ones(length(theta(options_ami.sens_ind)),1)),[4,3,2,1]));
    sol.s2rz = bsxfun(@times,s2rz,permute(theta(options_ami.sens_ind)*transpose(theta(options_ami.sens_ind))*(log(10)^2),[4,3,2,1])) + bsxfun(@times,srz,permute(diag(log(10)^2*theta(options_ami.sens_ind).*ones(length(theta(options_ami.sens_ind)),1)),[4,3,2,1]));
end
